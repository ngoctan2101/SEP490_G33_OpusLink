...
namespace OpusLink.User.Hosted.Pages.JOB
{
    public class FreelancerViewAllJobSavedPageModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            ...
            HttpResponseMessage response = await client.PostAsync(UrlConstant.ApiBaseUrl+"/Job6API/GetAllJobSaved/" + userID, httpContent);
        ...
        public async Task<IActionResult> OnPostForUnSaveAsync(IFormCollection collection)
        {
            ...
            //unsave it
            HttpResponseMessage response = await client.DeleteAsync(UrlConstant.ApiBaseUrl + "/Job7API/DeleteSaveJob/" + SaveJobId);
            if (response.IsSuccessStatusCode)
            { }
            string json = System.Text.Json.JsonSerializer.Serialize<Filter>(filter, options);
            StringContent httpContent = new StringContent(json, System.Text.Encoding.UTF8, "application/json");
            response = await client.PostAsync(UrlConstant.ApiBaseUrl + "/Job6API/GetAllJobSaved/" + userID, httpContent);
            if (response.IsSuccessStatusCode)
            {
                string strData = await response.Content.ReadAsStringAsync();
                SavedJobs = JsonConvert.DeserializeObject<List<GetSaveJobResponse>>(strData);
        ...
        {
            //get all category
            HttpResponseMessage response = await client.GetAsync(UrlConstant.ApiBaseUrl + "/Job6API/GetAllCategory");
            if (response.IsSuccessStatusCode)
            {
                string strData = await response.Content.ReadAsStringAsync();
