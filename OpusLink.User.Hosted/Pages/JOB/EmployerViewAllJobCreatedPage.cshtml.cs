...
namespace OpusLink.User.Hosted.Pages.JOB
{
    public class EmployerViewAllJobCreatedPageModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            ...
            HttpResponseMessage response = await client.PostAsync(UrlConstant.ApiBaseUrl+"/Job8API/GetAllJob", httpContent);
        ...
        {
            //get all category has parent is 0
            HttpResponseMessage response = await client.GetAsync(UrlConstant.ApiBaseUrl+"/Job8API/GetAllChildCategory?parentId=" + 0);
            if (response.IsSuccessStatusCode)
            {
                string strData = await response.Content.ReadAsStringAsync();
        ...
        {
            //get all category
            HttpResponseMessage response = await client.GetAsync(UrlConstant.ApiBaseUrl + "/Job8API/GetAllCategory");
            if (response.IsSuccessStatusCode)
            {
                string strData = await response.Content.ReadAsStringAsync();
                return JsonConvert.DeserializeObject<List<GetCategoryResponse>>(strData);
